...
namespace SqlStrings{
    /*SELECT AVG(%s)
      FROM (SELECT %s
            ...
            FROM Series))*/
    static const char* medianStr = "SELECT AVG(%s) FROM (SELECT %s FROM Series where %s is not null ORDER BY %s LIMIT 2 - (SELECT COUNT(%s) FROM Series) % 2 OFFSET (SELECT (COUNT(%s) - 1) / 2 FROM Series))";
    static const int numToFormatMedianStr = 6;

    static const std::string timeWatchedSql =
        "SELECT *\n"
        "FROM\n"
        "(\n"
    ...
    static const std::string timeRewatchedSql =
        "SELECT *\n"
        "FROM\n"
        "(\n"
